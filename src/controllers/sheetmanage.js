...
const sheetmanage = {
  ...
  getCurSheet: function() {
    if (Store.luckysheetfile.length) {
      ...
      if (!hasActive) {
        Store.luckysheetfile[index].status = 1;
      }
    }
  ...
  setSheetHide: function(index, isDelete) {
    ...
    if (
      !isDelete &&
      !method.createHookFunction('sheetHideBefore', { sheet: Store.luckysheetfile[currentIdx] })
    ) {
      return;
    }
      ...
      if (Store.currentSheetIndex == sheetIndex) {
        ...
        btn.push(
          replaceHtml(sheetHTML, {
            ...
            style: Store.luckysheetfile[i].hide == 1 ? 'display:none;' : '',
            colorset: colorset,
          }),
        );
      } else {
    ...
    setTimeout(function() {
      ...
      let ini = function() {
        ...
        if (loadSheetUrl == '') {
          ...
          $.post(loadSheetUrl, { gridKey: server.gridKey, index: sheetindex.join(',') }, function(
            d,
          ) {
  ...
  storeSheetParam: function() {
    ...
    if (
      $('#luckysheet-scrollbar-y')[0].scrollHeight > $('#luckysheet-scrollbar-y')[0].offsetHeight
    ) {
  ...
  changeSheet: function(index, isPivotInitial, isNewSheet, isCopySheet) {
    ...
    $(
      '#luckysheet-filter-selected-sheet' + index + ', #luckysheet-filter-options-sheet' + index,
    ).show();
  ...
  setCurSheet: function(index) {
    for (let i = 0; i < Store.luckysheetfile.length; i++) {
      if (Store.luckysheetfile[i]['index'] == index && !Store.luckysheetfile[i].hide) {
  ...
  restoreFilter: function(sheetIndex) {
    ...
    $('#luckysheet-filter-options-sheet' + sheetIndex + ' .luckysheet-filter-options').each(
      function(i) {
        if (file.filter == null) {
          return false;
        }
