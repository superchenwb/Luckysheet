...
import { onClickPrint } from '../expendPlugins/print/plugin';
  ...
  initialMenuButton: function() {
      ...
      if ($menuButton.length == 0) {
        ...
        $menuButton.find('.luckysheet-cols-menuitem').click(function() {
          ...
          if (itemvalue == 'freezenRow') {
            ...
            // 固定超出屏幕范围
            let rangeTop =
              Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1].top;
            if (
            ...
            // 固定超出屏幕范围
            let rangeLeft =
              Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1].left;
            if (
              luckysheetFreezen.freezenRealFirstRowColumn &&
            ...
            let last = Store.luckysheet_select_save[Store.luckysheet_select_save.length - 1];
            let column_focus =
              last['column_focus'] == null ? last['column'][0] : last['column_focus'];
    ...
    $('#luckysheet-icon-conditionformat').click(function() {
      ...
      if ($menuButton.length == 0) {
        ...
        $('body')
          .first()
          .append(menu + submenu + submenu2 + submenu3 + submenu4 + submenu6);
    ...
    for (let s = 0; s < Store.luckysheet_select_save.length; s++) {
      ...
      if (st_r == ed_r && st_c == ed_c) {
        ...
        for (let r = st_r; r <= ed_r; r++) {
          r_false =
            _this.singleFormulaInput(d, col_index, r, st_c, ed_c, formula, 'r', true, false) &&
            r_false;
        }
        ...
        for (let c = st_c; c <= ed_c; c++) {
          c_false =
            _this.singleFormulaInput(d, row_index, c, st_r, ed_r, formula, 'c', true, false) &&
            c_false;
        }
